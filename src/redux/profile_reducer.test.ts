...
test( "length of post should be incremented",() => {
	...
	//2. action
	let newStets = profileReducer(state,action)
	//3. expectation
	expect(newStets.posts.length).toBe(5)
} )
...
test( "message of new post should be corrected",() => {
	...
	let newStets = profileReducer(state,action)
	//3. expectation
	expect(newStets.posts[4].message).toBe('it-kamasutra.com')
} )
...
test( "after deleting length of message should be decrement",() => {
	...
	let newStets = profileReducer(state,action)
	//3. expectation
	expect(newStets.posts.length).toBe(3)
} )
...
test("the title should change",() => {
	...
	let newStets = profileReducer(state,action)
	//3. expectation
	expect(newStets.newPostText).toBe("test_text_corrected")
} )
...
test("the status should be established",() => {
	...
	let newStets = profileReducer(state,action)
	//3. expectation
	expect(newStets.status).toBe("test_text_corrected")
} )
...
test("profile should set data",() => {
	...
	let newStets = profileReducer(state,action)
	//3. expectation
	expect(newStets.profile.length).toBe(1)
} )
...
test("status should update",() => {
	...
	let newStets = profileReducer(state,action)
	//3. expectation
	expect(newStets.status).toBe("status_test_text")
} )
